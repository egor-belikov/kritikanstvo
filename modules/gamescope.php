...
if($id!= null & $url != null)
{
	$link = 'http://www.gamescope.ru/category/reviews-ru/';
	$page=file_get_contents($link);
  	// $page = mb_convert_encoding($page, "UTF-8", "windows-1251");
	$content_html = SelectNode(SelectNode($page, 'div', 'class="gsTable reviews"'),'ol');

	$items = SelectNodes($content_html, 'div','class="parent_for_identical_height"');
	foreach ($items as $item)
	{
		$url_div = SelectNode(SelectNode($item, 'div', 'class="col-title"'),'li');
		// $url_div = $item;
