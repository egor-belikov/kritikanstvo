...
if($id!= null & $url != null)
{
	$link = "http://25-k.com/list.php?c=novinki";
	$page=file_get_contents($link);
  	$page = mb_convert_encoding($page, "UTF-8", "windows-1251");
	$content_html = SelectNode($page, "table", "class=\"cells\"");

	$items = SelectNodes($content_html, "tr");
	foreach ($items as $item)
	{
		$url_div = SelectNode($item, "strong");
		if ($url_div!='')
		{
			$new_url = 'http://25-k.com/'.SelectiSubs($url_div, "href=\"", "\"");
