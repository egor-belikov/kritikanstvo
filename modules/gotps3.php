...
if($id!= null & $url != null)
{
	$link = 'http://gotps3.ru/reviews/';
	$page=file_get_contents($link);
  	// $page = mb_convert_encoding($page, "UTF-8", "windows-1251");
	$content_html = SelectNode($page, 'div', 'class="CenterCont"');

	$items = SelectNodes($content_html, 'div', 'class="ContentWindow"');
	foreach ($items as $item)
	{
		$url_div = SelectNode($item, 'div', 'class="NewsCaption"');
		// $url_div = $item;
		$new_url = SelectiSubs($url_div, 'href="', '"');	
		if ($new_url!='')
		{
			$new_url='http://gotps3.ru'.$new_url;
			$new_text = trim(htmlspecialchars_decode(str_replace(array('&laquo;','&raquo;', '&nbsp;', "
	", '&#8212;', '&mdash;', '&quot;', 'gotReview: '),array('', '', ' ', '', '—', '—', '"', ''),strip_tags($url_div))));
