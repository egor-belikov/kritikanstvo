...
if($id!= null & $url != null)
{
	$link = 'http://lampcoffeegame.com/category/chashka-kofe/reviews/';
	$page=file_get_contents($link);
  	// $page = mb_convert_encoding($page, "UTF-8", "windows-1251");
	$content_html = SelectNode(SelectNode($page, 'div', 'class="content"'),'div','class="article"');

	$items = SelectNodes($content_html, 'article');
	foreach ($items as $item)
	{
		$url_div = SelectNode(SelectNode($item, 'header'), 'h2');
		// $url_div = $item;
