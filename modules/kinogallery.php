...
if($id!= null & $url != null)
{
	$link = "http://www.kinogallery.com/reviews.php";
	$page=file_get_contents($link);
  	$page = mb_convert_encoding($page, "UTF-8", "windows-1251");
	$content_html = SelectNode($page, "div", "class=\"review_list\"");

	$items = SelectNodes($content_html, "li");
	foreach ($items as $item)
	{
		$url_div = SelectNode($item, "div", "class=\"review_list_link\"");
		$new_url = SelectiSubs($url_div, "href=\"", "\"");	
		if ($new_url!='')
		{
			$new_url='http://www.kinogallery.com'.$new_url;
