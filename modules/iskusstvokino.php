...
if($id!= null & $url != null)
{
	$link = "http://kinoart.ru/blogs?limitstart=0";
	$page=file_get_contents($link);
  	// $page = mb_convert_encoding($page, "UTF-8", "windows-1251");
	$content_html = SelectNode($page, "section", "class=\"blog\"");

	$items = SelectNodes($content_html, "div", "class=\"items-row");
	foreach ($items as $item)
	{
		$url_div = SelectNode(SelectNode(SelectNode(SelectNode($item, "div", "class=\"column-1\""),"article"),"header"),'h1');
		$new_url = 'http://kinoart.ru'.SelectiSubs($url_div, "href=\"", "\"");

		$new_text = trim(htmlspecialchars_decode(str_replace(array('&laquo;','&raquo;','&nbsp;',"
"),'',strip_tags($url_div.'— «'.between(SelectNode(SelectNode(SelectNode($item, "div", "class=\"column-1\""),"article"),"p"),'«','»').'»'))));
