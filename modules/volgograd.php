...
if($id!= null & $url != null)
{
	$link = "http://www.volgograd.ru/theme/info/culture/movie/";
	$page=file_get_contents($link);
  	// $page = mb_convert_encoding($page, "UTF-8", "windows-1251");
	$content_html = SelectNode($page, "div", "class=\"p-list\"");

	$items = SelectNodes($content_html, "dl");
	foreach ($items as $item)
	{
		$url_div = SelectNode(SelectNode($item, "dd"),'h4');
		$new_url = SelectiSubs($url_div, "href=\"", "\"");	
		if ($new_url!='')
		{
			$new_url='http://www.volgograd.ru/'.$new_url;
