...
if($id!= null & $url != null)
{
	$link = 'http://www.stratege.ru/xml_reviews.php?apikey=fa4567ca67bd98g9ga67812fa0a5cdaf2';
	$page=file_get_contents($link);
  	// $page = mb_convert_encoding($page, "UTF-8", "windows-1251");
	$content_html = SelectNode($page, 'root');

	$items = SelectNodes($content_html, 'review');
	foreach ($items as $item)
	{
		$url_div = SelectNode($item, 'link');
		// $url_div = $item;
...
", '&#8212;', '&mdash;', '&quot;', '&#8217;',
			''),array('', '', ' ', '', '—', '—', '"', "'",
			''),strip_tags(SelectNode($item,'original')))));
