...
if($id!= null & $url != null)
{
	...
	{
		$url_div = SelectNode($item, 'div', 'class="post-article"');
		// $url_div = $item;
		$new_url = SelectiSubs(SelectNode($url_div,'h2'), 'href="', '"');	
		if ($new_url!='')
		{
			$new_url=''.$new_url;
...
", '&#8212;', '&mdash;', '&quot;', '&#8217;',
			''),array('', '', ' ', '', '—', '—', '"', "'",
			''),strip_tags(SelectNode($url_div,'h2').': '.SelectNode($url_div,'p')))));
