...
if($id!= null & $url != null)
{
	$link = "http://games.mail.ru/pc/articles/review/";
	$page=file_get_contents($link);
  	// $page = mb_convert_encoding($page, "UTF-8", "windows-1251");
	$content_html = SelectNode($page, "div", "class=\"articles-pc-i\"");

	$items = SelectNodes($content_html, "div", "class=\"one ");
	foreach ($items as $item)
	{
		$url_div = SelectNode($item,'div','class="hdr"');
		$new_url = SelectiSubs($url_div, "href=\"", "\"");	
		if ($new_url!='' && mb_strstr($item,'Редакция: '))
		{
			$new_url='http://games.mail.ru'.$new_url;
