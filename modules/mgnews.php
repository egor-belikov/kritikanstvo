...
if($id!= null & $url != null)
{
	$link = 'http://mgnews.ru/articles';
	$page=file_get_contents($link);
  	// $page = mb_convert_encoding($page, "UTF-8", "windows-1251");
	$content_html = SelectNode(SelectNode($page, 'div', 'class="content"'),'div','class="left-part"');

	$items = SelectNodes($content_html, 'div', 'class="item"');
	foreach ($items as $item)
	{
		$url_div = SelectNode($item, 'h1');
		// $url_div = $item;
		$new_url = SelectiSubs($url_div, 'href="', '"');	
		if ($new_url!='')
		{
			$new_url='http://mgnews.ru'.$new_url;
			$new_text = trim(htmlspecialchars_decode(str_replace(array('&laquo;','&raquo;', '&nbsp;', "
	", '&#8212;', '&mdash;', '&quot;',' [Голосование]'),array('', '', ' ', '', '—', '—', '"',''),strip_tags($url_div))));
