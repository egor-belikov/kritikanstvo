...
if($id!= null & $url != null)
{
	$link = 'http://gameslife.ru/review/index.html';
	$page=file_get_contents($link);
  	// $page = mb_convert_encoding($page, "UTF-8", "windows-1251");
	$content_html = SelectNode($page, 'div', 'class="bcontent"');

	$items = SelectNodes($content_html, 'div', 'class="unit article');
	foreach ($items as $item)
	{
		$url_div = SelectNode($item, 'h5');
		// $url_div = $item;
		$new_url = SelectiSubs($url_div, "href='", "'");	
		if ($new_url!='')
		{
			$new_url='http://gameslife.ru'.$new_url;
