...
if($id!= null & $url != null)
{
	$link = 'http://www.gq.ru/blogs/revizor/?PAGEN_1';
	$page=file_get_contents($link);
  	// $page = mb_convert_encoding($page, "UTF-8", "windows-1251");
	$content_html = SelectNode($page, 'div', 'class="list"');

	$items = SelectNodes($content_html, 'div', 'class="article"');
	foreach ($items as $item)
	{
		...
		if ($new_url!='')
		{
			$new_url='http://www.gq.ru'.$new_url;
			$new_text = trim(htmlspecialchars_decode(str_replace(array('&laquo;','&raquo;', '&nbsp;', "
", '&#8212;', '&mdash;', '&quot;', '&#8217;',
			''),array('', '', ' ', '', '—', '—', '"', "'",
			''),strip_tags($url_div.': '.SelectNode($item,'p','class="desc"')))));
