...
if($id!= null & $url != null)
{
	$link = 'http://vozduh.afisha.ru/cinema/';
	$page=file_get_contents($link);
  	// $page = mb_convert_encoding($page, "UTF-8", "windows-1251");
	$content_html = SelectNode($page, 'section', 'class="articles"');

	$items = SelectNodes($content_html, 'article', 'class="articles_item');
	foreach ($items as $item)
	{
		...
		if ($new_url!='')
		{
			$new_url='http://vozduh.afisha.ru'.$new_url;
