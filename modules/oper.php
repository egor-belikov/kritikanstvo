...
if($id!= null & $url != null)
{
	$link = 'http://kino.oper.ru/torture/list.php?div=631';
	$page=file_get_contents($link);
  	$page = mb_convert_encoding($page, "UTF-8", "KOI8-R");
	$content_html = SelectNode($page, 'div', 'id="wrapper"');
	$items = SelectNodes($content_html, 'h1');
	foreach ($items as $item)
	{
		...
		if ($new_url!='')
		{
			$new_url='http://kino.oper.ru'.$new_url;
